...
def calculate_subsidy(zone, enterprise_size, plant_machinery, building_civil_work,
    ...
    #Check if land is legal entity
    if data.get("land_owned_by_legal_entity", "no").lower() == "yes":
        land_cost = float(data.get("land_cost", 0.0))

    stamp_duty_subsidy = zone_info["Stamp Duty (%)"] * (0.07 * land_cost)

    # Check if loan is availed
    if data.get("loan_avail", "no").lower() == "yes":
...
# === API Endpoint ===
@app.route("/calculate-subsidy", methods=["POST", "HEAD"])
def calculate():
