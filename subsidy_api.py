...
# API route to process data and return JSON response
...
def calculate():
    try:
        ...
        # Build a public URL for the PDF download
        pdf_filename = os.path.basename(result["pdf_path"])
        pdf_url = f"https://subsidy-api.onrender.com/download-pdf?filename={pdf_filename}"
...
# Separate endpoint to download the generated PDF
...
def download_pdf():
    ...
    pdf_path = os.path.join("generated_reports", filename)  # Assuming you save PDFs here
    ...
    return send_file(
        pdf_path,
        as_attachment=True,
        download_name="Subsidy_Calculation_Report.pdf",
