...
def generate_report_karnataka(user_data, result, zone):
    ...
    filename = f"{safe_name}_Karnataka_Subsidy_Report.pdf"
    tex_filename = f"{safe_name}_Karnataka_Subsidy_Report.tex"
...
\\begin{{itemize}}[leftmargin=1.5em]
    \\textbf{{1.Asset Creation Incentives (You can choose one from below)}} 
...
\\textbf{{(a)Interest Subsidy(applicable only when a term loan is availed for the project): }}In Karnataka interest subsidy is provided to MSMEs in Zone A and Zone B only to promote technology adoption and 
reduce the cost of finance for new and expanding enterprises. \\\\
\\\\
...
\\begin{{itemize}}[leftmargin=1.5em]
    \\item There will be a sanction provided for each of the subsidy application made which is sanctioned in upto 90 days and then disbursed as per funds availability with the Govt. Department and ranges from 3 months to 6 months from the date of sanction of the subsidy application.
...
\\section*{{Disclosure}}
\\begin{{itemize}}[leftmargin=1.5em]
  \\item SCPL (parent company of Subsidy4India) Team has calculated the subsidy based on details provided by the client and the same can vary depending on the capital investment 
        made by the client. exact location of the land where the manufacturing unit is being setup, documents provided for registering the subsidy application and any follow up 
        documents required by the Central or State Government authorities and will not be liable for any reduction in subsidy amount applicable to the client including the client being 
        determined as non-eligible to avail the subsidy due to lack of documentation, change of policy and non-cooperation by client
\\end{{itemize}}
