...
import pandas as pd
from report_haryana import generate_pdf
...
# Calculation
def subsidy_haryana(zone, enterprise_size, plant_machinery, building_civil_work,
...
def calculate():
    try:
        ...
        result = process_subsidy_application(data, df, generate_pdf, subsidy_haryana)

        if "error" in result:
            return jsonify({"error": result["error"]}), 400

        # Return both JSON result and a download link for PDF
        response_json = {
            "zone": result["zone"],
            "subsidy_breakup": result["subsidy_breakup"],
            "pdf_download_link": "/download_pdf"  # Added for demonstration
        }
...
# Additional route to download the generated PDF separately
